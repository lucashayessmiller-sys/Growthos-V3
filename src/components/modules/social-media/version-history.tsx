...
export function SocialVersionHistory({ post, onRestore }: { post: SocialPost; onRestore: (versionId: string) => void }) {
  const sorted = [...post.versions].sort((a, b) => +new Date(b.createdAt) - +new Date(a.createdAt))

  return (
    <Sheet>
      <SheetTrigger asChild>
        <Button variant="outline" size="sm"><History className="h-4 w-4" /> History ({post.versions.length})</Button>
      </SheetTrigger>
      <SheetContent className="w-full overflow-y-auto sm:max-w-md">
        <SheetHeader><SheetTitle>Version history</SheetTitle></SheetHeader>
        <div className="mt-4 space-y-3">
          {sorted.map((v) => {
            const active = v.id === post.activeVersionId
            return (
              <div key={v.id} className={cn("rounded-lg border p-3", active ? "border-primary bg-primary/5" : "border-border")}>
                ...
                <p className="mt-2 text-xs text-muted line-clamp-3 whitespace-pre-wrap">{v.caption}</p>
                <div className="mt-2 flex items-center justify-between">
                  <span className="text-[11px] text-muted">{v.note} · {v.hashtags.length} hashtags</span>
                  {!active && (
